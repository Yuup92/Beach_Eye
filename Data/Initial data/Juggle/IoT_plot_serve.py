...
with open('gyro_1.txt') as csvfile:
	reader = csv.reader(csvfile, delimiter=',')
	for line in reader:
...
ax1.set(xlabel='time (ms)', ylabel='Accel. $(m/s^{2})$',
...
ax2.set(xlabel='time (ms)', ylabel='Accel.  $(m/s^{2})$')
plt.setp(ax2.get_xticklabels(), fontsize=6)
ax2.legend(loc="upper right")

ax3=plt.subplot(313, sharex=ax1)
ax3.plot(time,zd, 'g', label='Z-axis')
ax3.set(xlabel='time (ms)', ylabel='Accel.  $(m/s^{2})$')
...
ax = plt.subplot(111)
ax.set(xlabel='time (s)', ylabel='Degrees per Second (deg/s)', title='Bouncing a ball')
